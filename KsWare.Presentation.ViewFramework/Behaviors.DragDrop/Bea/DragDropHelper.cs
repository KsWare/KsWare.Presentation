...
namespace KsWare.Presentation.ViewFramework.Behaviors.DragDrop.Bea {
	...
	public class DragDropHelper {
		...
		private static DragDropHelper Instance {
			get {
				if (s_Instance == null) { s_Instance = new DragDropHelper(); }
				return s_Instance;
			}
		}
		...
		public static bool GetIsDragSource(DependencyObject obj) { return (bool) obj.GetValue(IsDragSourceProperty); }
		...
		public static bool GetIsDropTarget(DependencyObject obj) { return (bool) obj.GetValue(IsDropTargetProperty); }
		...
		public static DataTemplate GetDragDropTemplate(DependencyObject obj) { return (DataTemplate) obj.GetValue(DragDropTemplateProperty); }

		public static void SetDragDropTemplate(DependencyObject obj, DataTemplate value) { obj.SetValue(DragDropTemplateProperty, value); }
		...
		private void DragSource_PreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e) {
			...
			if (this.sourceItemContainer != null) { this.draggedData = this.sourceItemContainer.DataContext; }
		}
		...
		private void DragSource_PreviewMouseLeftButtonUp(object sender, MouseButtonEventArgs e) { this.draggedData = null; }
		...
		private void DropTarget_PreviewDrop(object sender, DragEventArgs e) {
			...
			if (draggedItem != null) {
				if ((e.Effects & DragDropEffects.Move) != 0) { indexRemoved = Utilities.RemoveItemFromItemsControl(this.sourceItemsControl, draggedItem); }
				// This happens when we drag an item to a later position within the same ItemsControl.
				if (indexRemoved != -1 && this.sourceItemsControl == this.targetItemsControl && indexRemoved < this.insertionIndex) { this.insertionIndex--; }
			...
			if (IsDropDataTypeAllowed(draggedItem)) {
				if (targetItemsControlCount > 0) {
					...
					if (this.targetItemContainer != null) {
						...
						if (!this.isInFirstHalf) { this.insertionIndex++; }
					}
					else {
		...
		private bool IsDropDataTypeAllowed(object draggedItem) {
			...
			if (draggedItem != null) {
				if (collectionSource != null) {
					...
					else if (typeof (IList).IsAssignableFrom(collectionType)) { isDropDataTypeAllowed = true; }
					else { isDropDataTypeAllowed = false; }
				}
