...
namespace KsWare.Presentation.Core.Patterns {
	...
	public static class SetParentPattern {
		...
		public static void Execute<TParent>(ref TParent parentBackingFieldRef, TParent newParent, string propertyName) where TParent:class{ 
			...
			if(caller==typeof(SetPropertyWithParentPattern)) {
			    ...
			    var parentType0 = newParent != null ? newParent.GetType() : parentBackingFieldRef.GetType(); //ex: DerivedList
			    var parentType = parentType0;
			    while (true) {
	//FIXME:		if(dic.ContainsKey(parentType0) && dic[parentType0]==caller) break; //test with the chached type if available
			        var parentType2 = parentType.IsGenericType ? parentType.GetGenericTypeDefinition() : parentType;
			        if(parentType2==caller) {
			            // for performace cache the matching parent type
